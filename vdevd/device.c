...
static int vdev_device_request_make_env_str(char const *key, char const *value,
					    char **ret)
{
			       ...
			       size_t * num_env, int is_daemonlet)
{
	...
	if (vdev_path != NULL) {
		...
		rc = vdev_device_request_make_env_str("VDEV_PATH", vdev_path,
						      &env[i]);
		if (rc != 0) {
	...
	if (req->dev != 0) {
		...
		rc = vdev_device_request_make_env_str("VDEV_MAJOR", dev_buf,
						      &env[i]);
		if (rc != 0) {
		...
		rc = vdev_device_request_make_env_str("VDEV_MINOR", dev_buf,
						      &env[i]);
		if (rc != 0) {
	...
	rc = vdev_device_request_make_env_str("VDEV_LOGLEVEL", loglevel,
					      &env[i]);
	if (rc != 0) {
	...
	for (dp = sglib_vdev_params_it_init_inorder(&itr, req->params);
		...
		rc = vdev_device_request_make_env_str(varname, param_value,
						      &env[i]);
...
// return 0 on success
// return negative on error 
static int vdev_device_put_metadata_item(char *base_dir, char const *param_name,
...
// return negative on error 
// NOTE: not reload-safe; call while the reload lock is held
static int vdev_device_mkdirs(struct vdev_device_request *req,
			      char **dev_fullpath)
{
			...
			if (rc != 0) {
				...
				vdev_error
				    ("vdev_action_run_commands(ADD %s, dev=(%u, %u)) rc = %d\n",
				     req->renamed_path, major(req->dev),
				     minor(req->dev), rc);
			}
		}
	}
...
int vdev_device_remove(struct vdev_device_request *req)
{
	...
	if (req->renamed_path != NULL) {
		...
		if (strcmp(req->renamed_path, VDEV_DEVICE_PATH_UNKNOWN) != 0) {
			...
			if (req->dev != 0 && req->mode != 0
			    && !vdev_config_has_OS_quirk(req->state->
							 config->OS_quirks,
							 VDEV_OS_QUIRK_DEVICE_EXISTS))
			{
