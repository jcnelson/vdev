...
static vdev_device_request_t vdev_linux_parse_device_request_type(char const
								  *type)
{
...
static int vdev_linux_sysfs_parse_device_nums(char const *devbuf,
					      unsigned int *major,
					      unsigned int *minor)
{
...
static int vdev_linux_sysfs_read_dev_nums(struct vdev_linux_context *ctx,
					  char const *devpath,
					  unsigned int *major,
					  unsigned int *minor)
{
...
static int vdev_linux_sysfs_read_subsystem(struct vdev_linux_context *ctx,
					   char const *devpath,
					   char **subsystem)
{
	...
	subsystem_path =
	    vdev_linux_sysfs_fullpath(ctx->sysfs_mountpoint, devpath,
				      "subsystem");
...
static int vdev_linux_log_uevent(char const *uevent_buf, size_t uevent_buf_len,
				 bool debug)
{
...
// return 0 on success
static int vdev_linux_parse_request(struct vdev_linux_context *ctx,
				    ...
				    char *nlbuf, ssize_t buflen)
{
	...
	if (devpath != NULL) {
		...
		if (!have_major || !have_minor) {
			...
			rc = vdev_linux_sysfs_read_dev_nums(ctx, devpath,
							    &major, &minor);

			if (rc == 0) {

				// yup!
				vdev_device_request_set_dev(vreq,
							    makedev(major,
								    minor));
		...
		if (subsystem == NULL) {
			...
			rc = vdev_linux_sysfs_read_subsystem(ctx, devpath,
							     &subsystem);
...
static int vdev_linux_find_sysfs_mountpoint(char *mountpoint,
					    size_t mountpoint_len)
{
	...
	while (1) {
		...
		if (strcmp(ment_ptr->mnt_type, "sysfs") == 0) {
			...
			// found!
			strncpy(mountpoint, ment_ptr->mnt_dir,
				mountpoint_len - 1);
			found = true;
			rc = 0;
			break;
...
static int vdev_linux_sysfs_read_uevent(char const *fp_uevent,
					char **ret_uevent_buf,
					size_t * ret_uevent_len)
{
	...
	if (fp_uevent != NULL) {
		...
		if (rc != 0) {
			...
			for (uevent_len = 0; uevent_len < uevent_buf_len;) {
				...
				uevent_len +=
				    strlen(uevent_buf + uevent_len) + 1;
			}
...
// return 0 on success
// return -ENOMEM on OOM
static int vdev_linux_uevent_append(char **ret_uevent_buf,
...
static int vdev_linux_sysfs_register_device(struct vdev_linux_context *ctx,
					    char const *fp_uevent)
{
	...
	// get uevent
	rc = vdev_linux_sysfs_read_uevent(fp_uevent, &uevent_buf,
					  &uevent_buf_len);
	if (rc != 0) {
...
// return 0 on success
// return negative on error
static int vdev_linux_sysfs_find_devices(struct vdev_linux_context *ctx,
					 struct sglib_cstr_vector *uevent_paths)
{
	...
	while (1) {
		...
		if (scan_context.uevent_path != NULL) {
			...
			// yup--remember it
			char *uevent_path =
			    vdev_strdup_or_null(scan_context.uevent_path);
			...
			rc = sglib_cstr_vector_push_back(uevent_paths,
							 uevent_path);
			if (rc < 0) {
...
static int vdev_linux_sysfs_register_devices(struct vdev_linux_context *ctx)
{
	...
	// process all devices
	for (unsigned long i = 0; i < sglib_cstr_vector_size(&uevent_paths);
	     i++) {
...
static int vdev_linux_context_init(struct vdev_os_context *os_ctx,
	...
	if (!os_ctx->coldplug_only) {
		...
		if (geteuid() == 0) {
			...
			if (rc < 0) {
				...
				rc = -errno;
				vdev_error
				    ("setsockopt(SO_RCVBUFFORCE) rc = %d\n",
				     rc);
...
static int vdev_linux_mountpoint_on_devtmpfs(char const *mountpoint)
{
	...
	while (1) {
		...
		if (strcmp(ment_ptr->mnt_type, "devtmpfs") == 0) {
			...
			// found!
			strncpy(devtmpfs_mountpoint, ment_ptr->mnt_dir,
				PATH_MAX);
			...
			size_t min_len =
			    MIN(devtmpfs_realpath_mountpoint_len,
