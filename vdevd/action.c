...
int vdev_action_add_var(struct vdev_action *act, char const *name,
			char const *value)
{
...
static int vdev_action_ini_parser(void *userdata, char const *section,
	...
	if (strcmp(section, VDEV_ACTION_NAME) == 0) {
		...
		if (strcmp(name, VDEV_ACTION_NAME_PATH) == 0) {
			...
			if (act->path == NULL) {
				...
				rc = vdev_match_regex_init(&act->path_regex,
							   act->path);
...
int vdev_action_postprocess(struct vdev_config *config, struct vdev_action *act)
{
	...
	if (act->command == NULL && act->helper != NULL) {
		...
		sprintf(act->command, "%s/%s", config->helpers_dir,
			act->helper);
...
int vdev_action_load_file(struct vdev_config *config, char const *path,
		     ...
		     struct vdev_action *act)
{
	...
	rc = vdev_action_init(act, VDEV_DEVICE_INVALID, NULL, NULL, NULL,
			      false);
	if (rc != 0) {
...
static int vdev_action_vector_free(struct sglib_vdev_action_vector *acts)
{
	...
	for (unsigned long i = 0; i < sglib_vdev_action_vector_size(acts); i++) {
		...
		struct vdev_action *act =
		    sglib_vdev_action_vector_at_ref(acts, i);
	...
	if (rc != 0) {
		...
		if (sglib_vdev_action_vector_size(&acts) == 0) {
			...
			*ret_num_acts = 0;
			...
			sglib_vdev_action_vector_yoink(&acts, ret_acts,
						       &len_acts);
...
int vdev_action_run_sync(struct vdev_device_request *vreq, char const *command,
			 ...
			 char **output, size_t max_output)
{
	...
	rc = vdev_device_request_to_env(vreq, helper_vars, &req_env, &num_env,
					0);
	if (rc != 0) {
			  ...
			  vdev_params * helper_vars, bool use_shell)
{
	...
	if (pid == 0) {
		...
		if (pid == 0) {
			...
			// run the command 
			if (use_shell) {
				execle("/bin/dash", "dash", "-c", command,
				       (char *)0, env);
			} else {
...
static int vdev_action_daemonlet_start(struct vdev_state *state,
				       struct vdev_action *act)
{
...
static int vdev_action_daemonlet_stop(struct vdev_action *act)
{
	...
	if (rc < 0) {
		...
		vdev_error("close(%d PID=%d name=%s) rc = %d\n",
			   act->daemonlet_stdin, act->daemonlet_pid, act->name,
			   rc);
		act->daemonlet_stdin = -1;
	...
	while (1) {
		...
		if (child_pid < 0) {
			...
			if (rc == -ECHILD) {
				...
				child_pid =
				    waitpid(act->daemonlet_pid, &rc, WNOHANG);
				if (child_pid > 0) {
...
static int vdev_action_daemonlet_send_command(struct vdev_device_request *vreq,
					      struct vdev_action *act,
					      int64_t * daemonlet_rc)
{
	...
	rc = vdev_action_daemonlet_read_int64(act->daemonlet_stdout,
					      daemonlet_rc);
	if (rc < 0) {
...
int vdev_action_run_daemonlet(struct vdev_device_request *vreq,
			      struct vdev_action *act)
{
	...
	while (num_attempts < 2) {
		...
		rc = vdev_action_daemonlet_send_command(vreq, act,
							&daemonlet_rc);
		if (rc != 0) {
			...
			if (rc == -EAGAIN) {
				...
				if (rc < 0) {
					...
					rc = vdev_action_daemonlet_start
					    (vreq->state, act);
					if (rc < 0) {
	...
	if (act->dev_params != NULL) {
		...
		for (dp =
			...
			if (match != NULL) {
				...
				if (act_param_value == NULL
				    || strlen(act_param_value) == 0) {

					continue;
				}
				// otherwise, compare the values
				if (strcmp(vreq_param_value, act_param_value) !=
				    0) {
...
// return negative on error
int vdev_action_find_next(struct vdev_device_request *vreq,
		...
		if (rc == (signed)(num_acts - act_offset)) {
			...
			if (!acts[i].is_daemonlet) {
				...
				if (acts[i].async) {
					...
					method =
					    "vdev_action_run_daemonlet_async";
				} else {
