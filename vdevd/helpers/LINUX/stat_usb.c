...
int main(int argc, char **argv)
{
	...
	if (devtype_str != NULL && strcmp(devtype_str, "usb_device") == 0) {
		...
		dev_if_packed_info(sysfs_base, packed_if_str,
	...
	if (if_class_num == 8) {
		...
		if (rc == 0) {
			...
			set_usb_iftype(type_str, if_class_num,
				       sizeof(type_str) - 1);
		}
	...
	// got the device path!
	// get the device info
	dev_if_packed_info(usb_device_path, packed_if_str,
			   sizeof(packed_if_str));
	...
	if ((protocol == 6 || protocol == 2)) {
		...
		if (rc != 0) {
			...
			// nope 
			log_error
			    ("WARN: unable to read vendor string from '%s'",
			     dev_scsi_path);
			free(dev_scsi_path);
			free(dev_scsi_sysname);
			goto fallback;
		...
		if (rc != 0) {
			...
			// nope 
			log_error("WARN: unable to read model from '%s'",
				  dev_scsi_path);
			free(dev_scsi_path);
			free(dev_scsi_sysname);
		...
		if (rc != 0) {
			...
			// nope 
			log_error("WARN: unable to read type from '%s'",
				  dev_scsi_path);
			free(dev_scsi_path);
			free(dev_scsi_sysname);
 ...
 fallback:
	...
	rc = vdev_sysfs_read_attr(usb_device_path, "idVendor", &attr,
				  &attr_len);
	if (rc == 0) {

		strncpy(vendor_id, attr,
			(attr_len <
			 sizeof(vendor_id) - 1 ? attr_len : sizeof(vendor_id) -
			 1));
	...
	rc = vdev_sysfs_read_attr(usb_device_path, "idProduct", &attr,
				  &attr_len);
	if (rc == 0) {
	...
	if (serial_str[0] == 0) {
		...
		rc = vdev_sysfs_read_attr(usb_device_path, "serial", &attr,
					  &attr_len);
		if (rc == 0) {
