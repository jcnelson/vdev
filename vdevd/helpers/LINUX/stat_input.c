 ...
 */
static void get_cap_mask(char const *sysfs_cap_path, unsigned long *bitmask,
			 size_t bitmask_len)
{
...
int main(int argc, char **argv)
{
	...
	snprintf(sysdev_path, 4096, "/sys/dev/char/%s:%s/subsystem", major,
		 minor);
	...
	for (int i = 0; caps[i] != NULL; i++) {
		...
		snprintf(full_sysfs_path, 4096, "%s/%s", capabilities_path,
			 caps[i]);
