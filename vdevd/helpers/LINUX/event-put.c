...
int make_event_path(uint64_t seqnum, char *event_path)
{
	...
	return snprintf(event_path, PATH_MAX, "%s/%" PRIu64, g_dev_events,
			seqnum);
}
...
int multicast_event(uint64_t seqnum, char *event_path)
{
	...
	do {
		...
		if (rc != 0) {
			...
			// I/O error 
			log_error("readdir_r('%s') rc = %d", event_queues_dir,
				  rc);
			break;
		}
		// skip . and ..
...
int main(int argc, char **argv)
{
	...
	if (!have_seqnum) {
		...
		seqnum =
		    (uint64_t) strtoull(seqnum_str + strlen("SEQNUM="), &tmp,
					10);
		if (seqnum == 0
		    && (tmp == seqnum_str + strlen("SEQNUM=")
			|| *tmp != '\n')) {
