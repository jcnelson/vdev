...
static int skip_subsystem(char const *devpath, char const *subsys, char **next)
{
	...
	while (true) {
		...
		rc = vdev_sysfs_read_subsystem(parent, &subsystem,
					       &subsystem_len);
		...
		// matched; walk up
		rc = vdev_sysfs_get_parent_device(cur_dev, &parent,
						  &parent_len);
		if (rc != 0) {
...
static int handle_scsi_fibre_channel(char const *parent, char **path,
				     char **new_parent)
{
...
static int handle_scsi_sas_wide_port(char const *parent, char **path,
				     char **new_parent)
{
...
static int handle_scsi_iscsi(char const *parent, char **path, char **new_parent)
{
	...
	// read the target 
	rc = vdev_sysfs_read_attr(sessiondev, "targetname", &target,
				  &target_len);
	...
	// look up address 
	rc = vdev_sysfs_read_attr(conndev, "persistent_address", &addr,
				  &addr_len);
	if (rc != 0) {
...
// return 0 on success
// return negative on failure 
static int scsi_read_host_info(char const *parent, char **hostdev, int *host,
...
// return negative on error.
static int handle_scsi_default(char const *parent, char **path,
			       char **new_parent)
{
	...
	while (1) {
		...
		// get host info 
		rc = scsi_read_host_info(parent, &hostdev, &host, &bus, &target,
					 &lun);
...
// return negative on error 
static int handle_scsi_hyperv(char const *parent, char **path,
			      char **new_parent)
{
	...
	// get the guid 
	rc = vdev_sysfs_read_attr(vmbusdev, "device_id", &guid_str,
				  &guid_str_len);
...
// return negative on failure.
static int handle_scsi(char const *parent, char **path, char **new_parent,
		       bool * supported_parent)
{
	...
	rc = vdev_sysfs_uevent_read_key(parent, "DEVTYPE", &devtype,
					&devtype_len);
	if (rc != 0) {
...
static int handle_cciss(char const *parent, char **path, char **new_parent)
{
	...
	rc = vdev_sysfs_get_sysname(parent, &parent_sysname,
				    &parent_sysname_len);
	if (rc != 0) {
		return rc;
	}
...
static void handle_scsi_tape(char const *dev, char **path)
{
	...
	if (strncmp(sysname, "nst", 3) == 0
	    && strchr("lma", sysname[3]) != NULL) {
...
static int handle_ccw(char const *parent, char const *dev, char **path,
		      char **new_parent)
{
		...
		if (subsys != NULL) {
			...
			if (strcmp(subsys, "scsi_tape") == 0) {
				...
				if (rc == 0) {
					...
					rc = skip_subsystem(parent, "serio",
							    &new_parent);
				}
			} else if (strcmp(subsys, "pci") == 0) {

				rc = vdev_sysfs_get_sysname(parent, &sysname,
							    &sysname_len);
				if (rc == 0) {
				...
				if (rc == 0) {
					...
					rc = skip_subsystem(parent, "platform",
							    &new_parent);
				}
