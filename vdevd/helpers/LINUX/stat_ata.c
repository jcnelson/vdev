			 ...
			 int *out_is_packet_device)
{
	...
	if (peripheral_device_type == 0x05) {
		...
		is_packet_device = 1;
		ret =
		    disk_identify_packet_device_command(fd, out_identify, 512);
...
int main(int argc, char **argv)
{
	...
	if (id.command_set_1 & (1 << 1)) {
		...
		vdev_property_add
		    ("VDEV_ATA_FEATURE_SET_SECURITY_ERASE_UNIT_MIN", buf);
		...
		if ((id.cfs_enable_1 & (1 << 1))) {	/* enabled */
			...
			if (id.dlf & (1 << 8)) {
				...
				vdev_property_add
				    ("VDEV_ATA_FEATURE_SET_SECURITY_LEVEL",
				     "maximum");
			} else {
		...
		if (id.dlf & (1 << 5)) {
			...
			vdev_property_add
			    ("VDEV_ATA_FEATURE_SET_SECURITY_ENHANCED_ERASE_UNIT_MIN",
			     buf);
		}
		if (id.dlf & (1 << 4)) {

			vdev_property_add
			    ("VDEV_ATA_FEATURE_SET_SECURITY_EXPIRE", "1");
		}
		if (id.dlf & (1 << 3)) {
		...
		if (id.dlf & (1 << 2)) {
			...
			vdev_property_add("VDEV_ATA_FEATURE_SET_SECURITY_LEVEL",
					  "1");
		}
	}
	...
	if (id.command_set_2 & (1 << 9)) {
		...
		vdev_property_add
		    ("VDEV_ATA_FEATURE_SET_AAM_VENDOR_RECOMMENDED_VALUE",
		     aam_buf);
	...
	if (id.command_set_2 & (1 << 3)) {
		...
		if ((id.cfs_enable_2 & (1 << 3))) {
			...
			vdev_property_add
			    ("VDEV_ATA_FEATURE_SET_APM_CURRENT_VALUE",
			     apm_cur_buf);
		}
	}
	...
	if (word != 0x0000 && word != 0xffff) {
		...
		if (word & (1 << 2)) {
			...
			vdev_property_add("VDEV_ATA_SATA_SIGNAL_RATE_GEN2",
					  "1");
		}
		if (word & (1 << 1)) {

			vdev_property_add("VDEV_ATA_SATA_SIGNAL_RATE_GEN1",
					  "1");
		}
	}
	...
	if ((word & 0xf000) == 0x5000) {
		...
		snprintf(wwn_buf, 99, "0x%04x%04x%04x%04x", identify.wyde[108],
			 identify.wyde[109], identify.wyde[110],
			 identify.wyde[111]);
