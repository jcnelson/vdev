...
static int dev_pci_onboard(struct netnames *names)
{
	...
	rc = vdev_sysfs_read_attr(names->pcidev, "acpi_index", &index,
				  &index_len);

	/* SMBIOS type 41 -- Onboard Devices Extended Information */
	if (rc < 0) {
		rc = vdev_sysfs_read_attr(names->pcidev, "index", &index,
					  &index_len);
	}
...
static int dev_pci_slot(char const *dev_path, struct netnames *names)
{
	...
	if (domain > 0) {
		...
		sprintf(names->pci_path + strlen(names->pci_path), "P%u",
			domain);
	}
	...
	if (dev_port > 0) {
		...
		sprintf(names->pci_path + strlen(names->pci_path), "d%u",
			dev_port);
	}
...
static int names_bcma(char const *devpath, struct netnames *names)
{
	...
	if (core > 0) {
		snprintf(names->bcma_core, sizeof(names->bcma_core), "b%u",
			 core);
	}
	...
	if (names.type == NET_PCI) {
		...
		if (names.pci_onboard_label) {
			if (snprintf
				...
				vdev_property_add("VDEV_NET_LABEL_ONBOARD",
						  str);
				is_net_device = true;
			}
		}
