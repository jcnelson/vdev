	...
	for (i = 0, j = 0; str[i] != '\0'; i++) {
		...
		if (seqlen > 1) {
			...
			i += (seqlen - 1);
			   ...
			   || !vdev_whitelisted_char_for_devnode(str[i],
								 NULL)) {
...
int vdev_sysfs_read_attr(char const *sysfs_device_path, char const *attr_name,
...
int vdev_sysfs_get_parent_with_subsystem_devtype(char const *sysfs_device_path,
	...
	while (1) {
		...
		*tmp = 0;
		...
		if (devtype_name != NULL) {
			...
			// get uevent 
			rc = vdev_read_file(uevent_path, &uevent_buf,
					    &uevent_len);
			if (rc < 0) {
						  ...
						  size_t * devpath_len)
{
	...
	if (strcmp(subsystem, "subsystem") == 0) {
		...
		if (rc == 0) {
			return vdev_sysfs_read_device_path(path_buf, devpath,
							   devpath_len);
		}
...
// return -ENOMEM if OOM
int vdev_sysfs_get_parent_device(char const *dev_path, char **ret_parent_device,
...
int vdev_sysfs_get_sysname(char const *devpath, char **ret_sysname,
				       ...
				       size_t * syspath_len)
{
	...
	if (S_ISCHR(mode)) {
		...
		// character device 
		sprintf(devpath, "%s/dev/char/%u:%u", sysfs_mountpoint, major,
			minor);
	} else if (S_ISBLK(mode)) {

		// block device 
		sprintf(devpath, "%s/dev/block/%u:%u", sysfs_mountpoint, major,
			minor);
	} else {
