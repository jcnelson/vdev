				 ...
				 unsigned char *buffer, unsigned int len)
{
	...
	if (buffer[3] > MODEL_LENGTH) {
		 ...
		 */
		if (strneq
		    ((char *)&buffer[VENDOR_LENGTH], dev_scsi->vendor,
		     VENDOR_LENGTH)) {
...
static int prepend_vendor_model(struct scsi_id_device *dev_scsi, char *serial)
{
	...
	if (ind != (VENDOR_LENGTH + MODEL_LENGTH)) {
		log_debug("%s: expected length %d, got length %d",
			  dev_scsi->kernel, (VENDOR_LENGTH + MODEL_LENGTH),
			  ind);
		return -1;
	}
	return ind;
				  ...
				  char *wwn_vendor_extension, char *tgpt_group)
{
	...
	for (id_ind = 0;
		 ...
		 */
		for (j = 4; j <= (unsigned int)page_83[3] + 3;
		     j += page_83[j + 3] + 4) {
			   ...
			   int *page_code)
{
	...
	*page_code = default_page_code;
	...
	while (retval == 0) {
		...
		default:
			log_error("Unknown or bad option '%c' (0x%x)", option,
				  option);
			retval = -1;
			break;
		}
	...
	if (dev_scsi.wwn[0] != '\0') {
		...
		if (dev_scsi.wwn_vendor_extension[0] != '\0') {
			...
			vdev_property_add("VDEV_SCSI_WWN_VENDOR_EXTENSION",
					  wwn_vendor_buf);
			vdev_property_add("VDEV_SCSI_WWN_WITH_EXTENSION",
					  wwn_buf);
		}
