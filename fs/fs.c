...
// always succeeds
static void vdevfs_scandirat_context_init(struct vdevfs_scandirat_context *ctx,
...
static int vdevfs_scandirat_context_callback(int dirfd, struct dirent *dent,
					     void *cls)
{
	...
	if (S_ISDIR(sb.st_mode)) {
		...
		// woo! save it 
		joined_path =
		    vdev_fullpath(ctx->parent_path, dent->d_name, NULL);
		if (joined_path == NULL) {
...
static int vdev_get_mountpoint(int fuse_argc, char **fuse_argv,
			       char **ret_mountpoint)
{
...
int vdevfs_init(struct vdevfs *vdev, int argc, char **argv)
{
	...
	rc = vdev_get_mountpoint(fuse_argc, fuse_argv,
...
int vdevfs_shutdown(struct vdevfs *vdev)
{
	...
	if (vdev->fs != NULL) {
		...
		fskit_unroute_detach(fskit_fuse_get_core(vdev->fs),
				     vdev->close_rh);
...
static int vdevfs_access_check(struct vdevfs *vdev,
		   ...
		   size_t num_dirents)
{
	...
	for (unsigned int i = 0; i < num_dirents; i++) {
		...
		rc = vdev_acl_apply_all(vdev->config, vdev->acls,
					vdev->num_acls, child_path, ps, uid,
					gid, &sb);
		if (rc < 0) {
