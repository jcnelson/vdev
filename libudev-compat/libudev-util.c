 ...
 * libudev-util.c
 * 
 * Forked from systemd/src/libudev/libudev-util.c on March 26, 2015
 * 
			       ...
			       char *result, size_t maxsize, int read_value)
{
	...
	if (read_value) {
		...
		if (val != NULL)
			...
			result[0] = '\0';
		log_debug("value '[%s/%s]%s' is '%s'", subsys, sysname, attr,
			  result);
	} else {
		size_t l;
		char *s;
		...
		log_debug("path '[%s/%s]%s' is '%s'", subsys, sysname, attr,
			  result);
	}
...
int util_resolve_sys_link(struct udev *udev, char *syspath, size_t size)
{
	...
	strscpyl(base, size - (base - syspath), "/", &link_target[back * 3],
		 NULL);
	return 0;
}
