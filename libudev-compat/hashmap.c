 ...
 * hashmap.c
 * 
 * Forked from systemd/src/shared/hashmap.c on March 26, 2015
 * 
...
static struct hashmap_base_entry *bucket_at_virtual(HashmapBase * h,
...
static unsigned hashmap_iterate_in_internal_order(HashmapBase * h, Iterator * i)
{
	...
	if (i->idx == IDX_FIRST) {
		...
		if (h->has_indirect) {
			i->idx =
			    skip_free_buckets(h, h->indirect.idx_lowest_entry);
...
static void reset_direct_storage(HashmapBase * h)
{
	...
	p = mempset(h->direct.storage, 0,
		    hi->entry_size * hi->n_direct_buckets);
					    ...
					    HASHMAP_DEBUG_PARAMS)
{
	...
	h = use_pool ? mempool_alloc0_tile(hi->mempool) : malloc0(hi->
								  head_size);
...
static int resize_buckets(HashmapBase * h, unsigned entries_add)
{
	...
	for (idx = 0; idx < old_n_buckets; idx++) {
		...
		do {
			 ...
			 */
			rehash_next =
			    hashmap_put_robin_hood(h, optimal_idx, &swap);
			n_rehashed++;
			       ...
			       const void *new_key, void *value)
{
	...
	if (idx_new != IDX_NIL)
		if (idx_old != idx_new) {
			...
			assert(old_key ==
			       bucket_at(HASHMAP_BASE(h), idx_old)->key);
		}
