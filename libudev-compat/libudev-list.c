 ...
 * libudev-list.c
 * 
 * Forked from systemd/src/libudev/libudev-list.c on March 26, 2015
 * 
...
static inline struct udev_list_entry *list_node_to_entry(struct udev_list_node
							 *node)
{
...
{
	udev_list_node_insert_between(&new->node, entry->node.prev,
				      &entry->node);
	new->list = entry->list;
}
