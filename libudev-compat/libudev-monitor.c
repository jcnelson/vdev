...
int udev_monitor_filter_update(struct udev_monitor *udev_monitor)
{
	...
	if (udev_list_get_entry(&udev_monitor->filter_tag_list) != NULL) {
		...
		udev_list_entry_foreach(list_entry,
			...
			uint64_t tag_bloom_bits =
			    util_string_bloom64(udev_list_entry_get_name
						(list_entry));
			...
			bpf_stmt(ins, &i, BPF_ALU | BPF_AND | BPF_K,
				 tag_bloom_hi);
	...
	if (udev_list_get_entry(&udev_monitor->filter_subsystem_list) != NULL) {
		...
		{
			unsigned int hash =
			    util_string_hash32(udev_list_entry_get_name
					       (list_entry));
			...
			if (udev_list_entry_get_value(list_entry) == NULL) {
				...
				hash =
				    util_string_hash32(udev_list_entry_get_value
						       (list_entry));
				bpf_jmp(ins, &i, BPF_JMP | BPF_JEQ | BPF_K,
					hash, 0, 1);
			}
...
static int passes_filter(struct udev_monitor *udev_monitor,
...
static struct udev_device *udev_monitor_receive_device_netlink(struct
							       udev_monitor
							       *udev_monitor)
{
 ...
 retry:
	...
	if (snl.nl.nl_groups == 0) {
		...
		if (udev_monitor->snl_trusted_sender.nl.nl_pid == 0 ||
		    snl.nl.nl_pid !=
		    udev_monitor->snl_trusted_sender.nl.nl_pid) {
...
static struct udev_device *udev_monitor_receive_device_fs(struct udev_monitor
							  *udev_monitor)
{
	struct udev_device *udev_device;
	struct msghdr smsg;
	...
	if (rc == 0) {
		...
		if (rc < 0) {
			...
			if (rc != -EAGAIN) {
				...
				log_error
				    ("udev_monitor_fs_push_events rc = %d\n",
				     rc);
				return NULL;
			} else {
			     ...
			     struct udev_device *udev_device)
{
	...
	log_debug("passed %zi bytes to netlink monitor %p", count,
		  udev_monitor);
	return count;
}
				      ...
				      const char *tag)
{
	...
	if (tag == NULL)
		return -EINVAL;
	if (udev_list_entry_add(&udev_monitor->filter_tag_list, tag, NULL) ==
	    NULL)
		return -ENOMEM;
	return 0;
}
