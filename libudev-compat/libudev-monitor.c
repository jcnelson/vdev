...
static bool udev_has_devtmpfs(struct udev *udev) {
        ...
        if (r < 0) {
                if (errno != EOPNOTSUPP) {
                   int errsv = errno;
                   log_debug("name_to_handle_at on /dev: %s", strerror(errsv));
                }
                return false;
        }
...
struct udev_monitor *udev_monitor_new_from_netlink_fd(struct udev *udev, const char *name, int fd)
{
        ...
        if (name == NULL)
                ...
                if (access("/run/udev/control", F_OK) < 0 && !udev_has_devtmpfs(udev)) {
                        log_debug("%s", "the udev service seems not to be active, disable the monitor");
                        group = UDEV_MONITOR_NONE;
                } else
        ...
        if (fd < 0) {
                ...
                if (udev_monitor->sock == -1) {
                   int errsv = errno;
                   log_debug("error getting socket: %s", strerror(errsv));
                   free(udev_monitor);
                   return NULL;
                }
        } else {
