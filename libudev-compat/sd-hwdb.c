 ...
 * sd-hwdb.c
 * 
 * Forked from systemd/src/libsystemd/sd-hwdb.c on March 26, 2015
 * 
...
static const struct trie_child_entry_f *trie_node_children(sd_hwdb * hwdb,
							   const struct
							   trie_node_f *node)
{
...
static const struct trie_value_entry_f *trie_node_values(sd_hwdb * hwdb,
							 const struct
							 trie_node_f *node)
{
...
static const struct trie_node_f *node_lookup_f(sd_hwdb * hwdb,
...
static int hwdb_add_property(sd_hwdb * hwdb, const char *key, const char *value)
{
	...
	r = ordered_hashmap_ensure_allocated(&hwdb->properties,
					     &string_hash_ops);
	if (r < 0)
		return r;
...
static int trie_search_f(sd_hwdb * hwdb, const char *search)
{
	...
	while (node) {
		...
		if (node->prefix_off) {
			...
			for (; (c = trie_string(hwdb, node->prefix_off)[p]);
			     p++) {
				if (c == '*' || c == '?' || c == '[')
					return trie_fnmatch_f(hwdb, node, p,
							      &buf,
							      search + i + p);
				if (c != search[i + p])
					return 0;
...
_public_ int sd_hwdb_new(sd_hwdb ** ret)
{
	...
	NULSTR_FOREACH(hwdb_bin_path, hwdb_bin_paths) {
		...
		if (hwdb->f) {
			...
			int errsv = -errno;
			log_debug("error reading %s: %s", hwdb_bin_path,
				  strerror(errsv));
			return errsv;
		}
	}
	...
	hwdb->map =
	    mmap(0, hwdb->st.st_size, PROT_READ, MAP_SHARED, fileno(hwdb->f),
		 0);
	if (hwdb->map == MAP_FAILED) {

		int errsv = errno;
		log_debug("error mapping %s: %s", hwdb_bin_path,
			  strerror(errsv));
		return errsv;
	}
