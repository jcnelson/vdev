 ...
 * libudev-device.c
 * 
 * Forked from systemd/src/libudev/libudev-device.c on March 26, 2015
 * 
...
static int udev_device_set_seqnum(struct udev_device *udev_device,
				  unsigned long long int seqnum)
{
	char num[32];
...
static int udev_device_set_devpath_old(struct udev_device *udev_device,
...
static int udev_device_set_driver(struct udev_device *udev_device,
				  const char *driver)
{
...
static int udev_device_set_devtype(struct udev_device *udev_device,
				   const char *devtype)
{
...
static int udev_device_set_subsystem(struct udev_device *udev_device,
...
static int udev_device_set_devnode_mode(struct udev_device *udev_device,
					mode_t mode)
{
	char num[32];
...
static int udev_device_set_devnode_uid(struct udev_device *udev_device,
				       uid_t uid)
{
	char num[32];
...
static int udev_device_set_devnode_gid(struct udev_device *udev_device,
				       gid_t gid)
{
	char num[32];
...
int udev_device_add_property(struct udev_device *udev_device, const char *key,
			     const char *value)
{
...
static struct udev_list_entry *udev_device_add_property_from_string(struct
								    udev_device
								    *udev_device,
								    const char
								    *property)
...
static int udev_device_set_syspath(struct udev_device *udev_device,
				   const char *syspath)
{
	const char *pos;
	size_t len;
...
static void udev_device_set_usec_initialized(struct udev_device *udev_device,
					     usec_t usec_initialized)
{
...
static int udev_device_set_action(struct udev_device *udev_device,
				  const char *action)
{
 ...
 */
static void udev_device_add_property_from_string_parse(struct udev_device
						       *udev_device,
						       const char *property)
{
	if (startswith(property, "DEVPATH=")) {
		...
		udev_device_set_seqnum(udev_device,
				       strtoull(&property[7], NULL, 10));
...
static int udev_device_add_property_from_string_parse_finish(struct udev_device
							     *udev_device)
{
	if (udev_device->maj > 0)
		udev_device_set_devnum(udev_device,
				       makedev(udev_device->maj,
					       udev_device->min));
	udev_device->maj = 0;
	udev_device->min = 0;
...
static int udev_device_read_db(struct udev_device *udev_device)
{
	...
	if (f == NULL) {
		int errsv = errno;
		log_debug("no db file to read %s: %s", filename,
			  strerror(errsv));
		return errsv;
	}
	...
	while (fgets(line, sizeof(line), f)) {
		...
		switch (line[0]) {
		case 'S':
			strscpyl(filename, sizeof(filename), "/dev/", val,
				 NULL);
			udev_device_add_devlink(udev_device, filename);
			break;
		case 'L':
			udev_device_set_devlink_priority(udev_device,
							 atoi(val));
			break;
		case 'E':
			entry =
						   ...
						   const char *id)
{
	...
	case 'c':
		if (sscanf(id, "%c%i:%i", &type, &maj, &min) != 3)
			return NULL;
		return udev_device_new_from_devnum(udev, type,
						   makedev(maj, min));
	case 'n':{
			int sk;
			struct ifreq ifr;
			...
			dev =
			    udev_device_new_from_subsystem_sysname(udev, "net",
								   ifr.
								   ifr_name);
			if (dev == NULL)
				return NULL;
	...
	case '+':
		...
		sysname[0] = '\0';
		sysname = &sysname[1];
		return udev_device_new_from_subsystem_sysname(udev, subsys,
							      sysname);
	default:
		errno = EINVAL;
		return NULL;
...
struct udev_device *udev_device_new_from_environment(struct udev *udev)
{
	...
	for (i = 0; environ[i] != NULL; i++)
		udev_device_add_property_from_string_parse(udev_device,
							   environ[i]);
...
static struct udev_device *device_new_from_parent(struct udev_device
						  *udev_device)
{
								  ...
								  *devtype)
{
	...
	while (parent != NULL) {
		...
		if (parent_subsystem != NULL
		    && streq(parent_subsystem, subsystem)) {
			if (devtype == NULL)
				break;
			parent_devtype = udev_device_get_devtype(parent);
			if (parent_devtype != NULL
			    && streq(parent_devtype, devtype))
				break;
		}
		...
		if (udev_device_get_tags_list_entry(udev_device) != NULL) {
			...
			udev_list_entry_foreach(list_entry,
			    ...
			    strpcpyl(&s, l,
				     udev_list_entry_get_name(list_entry), ":",
				     NULL);
			udev_device_add_property_internal(udev_device, "TAGS",
							  tags);
		}
	}
...
static int udev_device_sysattr_list_read(struct udev_device *udev_device)
{
	...
	for (dent = readdir(dir); dent != NULL; dent = readdir(dir)) {
		...
		udev_list_entry_add(&udev_device->sysattr_list, dent->d_name,
				    NULL);
		num++;
	}
	...
	if (udev_device->id_filename == NULL) {
		...
		if (major(udev_device_get_devnum(udev_device)) > 0) {
			...
			if (asprintf(&udev_device->id_filename, "%c%u:%u",
				     ...
				     minor(udev_device_get_devnum(udev_device)))
			    < 0)
...
ssize_t udev_device_get_properties_monitor_buf(struct udev_device * udev_device,
					       const char **buf)
{
	if (!udev_device->envp_uptodate)
		if (update_envp_monitor_buf(udev_device) != 0)
...
int udev_device_rename(struct udev_device *udev_device, const char *name)
{
	...
	if (interface) {
		...
		udev_device_add_property_internal(udev_device, "INTERFACE_OLD",
						  interface);
		udev_device_add_property_internal(udev_device, "INTERFACE",
						  name);
	}
...
struct udev_device *udev_device_shallow_clone(struct udev_device *old_device)
{
	...
	udev_device_set_subsystem(device,
...
struct udev_device *udev_device_new_from_nulstr(struct udev *udev, char *nulstr,
						ssize_t buflen)
{
