 ...
 * libudev-enumerate.c
 * 
 * Forked from systemd/src/libudev/libudev-enumerate.c on March 26, 2015
 * 
...
static int syspath_add(struct udev_enumerate *udev_enumerate,
		       const char *syspath)
{
	char *path;
...
static bool devices_delay_end(struct udev *udev, const char *syspath)
{
	...
	for (i = 0; delay_device_list[i] != NULL; i++) {
		if (strstr(syspath + strlen("/sys"), delay_device_list[i]) !=
		    NULL)
			return true;
	}
	return false;
						      ...
						      *udev_enumerate)
{
	...
	if (!udev_enumerate->devices_uptodate) {
		...
		for (i = 0; i < max; i++) {
			...
			if (prev != NULL &&
			    entry->len == prev->len &&
			    memcmp(entry->syspath, prev->syspath,
				   entry->len) == 0)
				continue;
			prev = entry;

			/* skip to be delayed devices, and add them to the end of the list */
			if (devices_delay_end
			    (udev_enumerate->udev, entry->syspath)) {
			...
			if (move_later == -1) {
				move_later_prefix =
				    devices_delay_later(udev_enumerate->udev,
							entry->syspath);
			...
			if ((move_later >= 0) &&
				...
				udev_list_entry_add(&udev_enumerate->
						    devices_list,
						    udev_enumerate->
						    devices[move_later].syspath,
						    NULL);
				move_later = -1;
			}
...
static int match_sysattr_value(struct udev_device *dev, const char *sysattr,
...
static bool match_sysattr(struct udev_enumerate *udev_enumerate,
...
static bool match_property(struct udev_enumerate *udev_enumerate,
			   struct udev_device *dev)
{
	struct udev_list_entry *list_entry;
	bool match = false;
	...
	udev_list_entry_foreach(list_entry,
		...
		udev_list_entry_foreach(property_entry,
					...
					(dev)) {
			const char *dev_key =
			    udev_list_entry_get_name(property_entry);
...
static bool match_tag(struct udev_enumerate *udev_enumerate,
		      struct udev_device *dev)
{
	struct udev_list_entry *list_entry;
...
static bool match_parent(struct udev_enumerate *udev_enumerate,
			 struct udev_device *dev)
{
	if (udev_enumerate->parent_match == NULL)
		return true;
	...
	return startswith(udev_device_get_devpath(dev),
			  udev_device_get_devpath(udev_enumerate->
						  parent_match));
}

static bool match_sysname(struct udev_enumerate *udev_enumerate,
			  const char *sysname)
{
	struct udev_list_entry *list_entry;
	...
	udev_list_entry_foreach(list_entry,
				udev_list_get_entry(&udev_enumerate->
						    sysname_match_list)) {
		if (fnmatch(udev_list_entry_get_name(list_entry), sysname, 0) !=
		    0)
			continue;
		return true;
	}
				    ...
				    const char *subdir2)
{
	...
	for (dent = readdir(dir); dent != NULL; dent = readdir(dir)) {
		...
		strscpyl(syspath, sizeof(syspath), path, "/", dent->d_name,
			 NULL);
		dev =
		    udev_device_new_from_syspath(udev_enumerate->udev, syspath);
		if (dev == NULL)
			continue;
...
static bool match_subsystem(struct udev_enumerate *udev_enumerate,
			    const char *subsystem)
{
	struct udev_list_entry *list_entry;
	...
	udev_list_entry_foreach(list_entry,
		...
		if (fnmatch(udev_list_entry_get_name(list_entry), subsystem, 0)
		    == 0)
			return false;
	}
	...
	if (udev_list_get_entry(&udev_enumerate->subsystem_match_list) != NULL) {
		...
		{
			if (fnmatch
			    (udev_list_entry_get_name(list_entry), subsystem,
			     0) == 0)
				return true;
		}
		return false;
		    ...
		    const char *subdir, const char *subsystem)
{
	...
	for (dent = readdir(dir); dent != NULL; dent = readdir(dir)) {
		...
		if (!match_subsystem
		    (udev_enumerate,
		     subsystem != NULL ? subsystem : dent->d_name))
			continue;
...
static int scan_devices_tags(struct udev_enumerate *udev_enumerate)
{
	...
	udev_list_entry_foreach(list_entry,
		...
		for (dent = readdir(dir); dent != NULL; dent = readdir(dir)) {
			...
			dev =
			    udev_device_new_from_device_id(udev_enumerate->udev,
							   dent->d_name);
			if (dev == NULL)
				continue;
			...
			if (!match_subsystem
				...
				goto nomatch;
			if (!match_sysname
			    (udev_enumerate, udev_device_get_sysname(dev)))
				goto nomatch;
			if (!match_parent(udev_enumerate, dev))
				goto nomatch;
			if (!match_property(udev_enumerate, dev))
...
static int parent_crawl_children(struct udev_enumerate *enumerate,
