	...
	for (int i = 0; need_fullpath[i] != NULL; i++) {
		...
		if (need_fullpath[i] != NULL && (*need_fullpath[i]) != NULL) {
			...
			if (*(need_fullpath[i])[0] != '/') {
				...
				// relative path 
				char *new_path =
				    vdev_fullpath(cwd_buf, *(need_fullpath)[i],
						  NULL);
				if (new_path == NULL) {
...
static int vdev_config_get_mountpoint_from_fuse(int fuse_argc, char **fuse_argv,
						char **ret_mountpoint)
{
