...
* **Container Friendly**.  Vdev can run in containers and jails, and offers the administrator an easy-to-understand way to restrict, reorder, and rewrite the device events the container will observe.  Unlike udev, the Linux port of vdev does not rely on netlink sockets to propagate events to client programs.
